...
module.exports = {    
  ...
  findStops: function(cb) { 
    Stop.find({}, cb);
  },

  findPlannedTrip: function(departure_stop_id, destination_stop_id, departure_date_time, cb) { 

    // So, how the fuck to do this?
    // Step #1: Find stop times with stop_id matching the departure_stop_id and departure_time
    // Step #2: Find stops times with stop_id matching the arrival_stop_id and trip id from #1
    // Step #3: Filter out results from #1 and #2 where the stop sequence is wrong
    // Done.

    mongoose.set('debug', true);

    var departure_js = new Date(departure_date_time);

    var createPossibleTimes = function(time) { 
      var times = [];
      var itTime = new Date(time);

      for (var i = 0; i < 61; i++) {
        times.push(utils.pad(itTime.getHours(), 2) + ':' + itTime.getMinutes() + ':00');
        itTime = new Date(itTime.getTime() + 60 * 1000);
      }

      return times;
    };

    var step1 = function(depart_stop, cb) { 
      StopTime.find({stop_id: departure_stop_id , departure_time: {'$in': createPossibleTimes(departure_js)}}, cb);
    };

    step1(departure_stop_id, function(e, stop_times) { 
      step2(destination_stop_id, stop_times, function(e, dsts) {
        console.log("dsts:" + _.map(dsts, function(d) { 
          return "from:" + d.start.departure_time + " to: " + d.destination.arrival_time + "\n";
        }));
      });
    });

    var step2 = function(destination_stop, starting_stop_times, cb) {
      var tripIds = _.map(starting_stop_times, function(st) { return st.trip_id; });

      StopTime.find({stop_id: destination_stop, trip_id: { '$in': tripIds}}, function(e, d_stop_times) { 
        if (e) { 
          cb(e, null);
        } else { 
          var matches = _.compact(_.map(d_stop_times, function(d_stop) {

            var matchingStopTime = _.compact(_.map(starting_stop_times, function(s_stop) {
              if (s_stop.trip_id == d_stop.trip_id) {
                var result = {
                  destination: d_stop,
                  start: s_stop
                };
            ...
            if (matchingStopTime.length > 1) {
              console.error('Found multiple instances of the same stop on a trip:' + d_stop);
            }

            if (matchingStopTime.length > 0) { return matchingStopTime[0]; } else { return null; };
          }));
  ...
  findTrip: function(route_id, calendar_id, cb) { 
    console.log("Finding: " + route_id + " - " + calendar_id);
    Trip.find({route_id: route_id, service_id: calendar_id}, cb);
  },
